...
function findUni(array){
  const uni = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].university.includes('Uni') = true){uni.push(array[i].university) }
  } return uni;
  }
  findUni(graduates);
...
const bothNames = `Name: ${arrayItem.animal_name}, Scientific: ${arrayItem.scientific_name}`;
...
const lowerCase = [];
zooAnimals.map(function(arrayItem) {
  let lowercased = arrayItem.animal_name.toLowerCase();
